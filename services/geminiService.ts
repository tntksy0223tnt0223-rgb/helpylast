...
export const getComprehensiveHealthAnalysis = async (
  ...
  const prompt = `
    ...
    **분석 리포트 형식:**
    ...
    2.  **주요 관찰 사항:** 건강 지표의 변화 추세, 생활 습관, 노트 내용을 종합하여 발견된 긍정적이거나 우려되는 점을 분석합니다. 특히 사용자의 주의가 필요한 위험 신호나 매우 부정적인 관찰 사항은 세 개의 별표(***)로 감싸서 강조해주세요. 예: ***혈압이 지속적으로 높게 측정되고 있습니다.***
...
export const getChatbotResponse = async (
  ...
  const systemInstruction = member
    ...
    사용자의 질문에 명확하고 공감적으로 답변하세요. 답변은 핵심 내용 위주로 2-3 문장 이내로 간결하게 해주세요.
    
    **경상도 사투리 이해 가이드:**
    사용자는 경상도 사투리를 사용할 수 있습니다. 아래 예시를 참고하여 사용자의 의도를 파악하고, 답변은 항상 친절한 표준어로 해주세요.
    - "파이다"는 "아니다" 또는 "좋지 않다"는 의미로 해석해주세요. (예: "몸이 파이다" -> "몸이 좋지 않다")
    - "기침이 나가에서"는 "기침이 나서"로 해석해주세요.
    - "목이 칼칼해가는"은 "목이 칼칼해서"로 해석해주세요.

    다음 건강 데이터를 참고할 수 있지만, 당신의 답변은 의학적 조언을 대체할 수 없음을 항상 명시하세요.
    - 기저 질환: ${member.chronicConditions || '없음'}
    ...
    사용자의 일반적인 건강 관련 질문에 명확하고 공감적으로 답변하세요. 답변은 핵심 내용 위주로 2-3 문장 이내로 간결하게 해주세요.
